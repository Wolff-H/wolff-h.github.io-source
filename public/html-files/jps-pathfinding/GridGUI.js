GridGUI = function (container, mapText) {
    ...
    self.search = Search_Student_JPS(self.map, self.config);
    self.pixelWidth = 640;
    ...
    self.draw = function () {
        ...
        if (self.omx != -1) {
            ...
            /*if (typeof self.search.getIgnore === 'function') {
            	ignore = self.search.getIgnore();
            	for (var i = 0; self.drawInfo && i <ignore.length; i++) {
            		self.drawAgent(ignore[i][0][0], ignore[i][0][1], 1, "#a9a9a9");
            	}
            }*/
    	...
    	for (var i = 0; self.drawInfo && i < ignore.length; i++) {
    		self.fg_ctx.lineWidth = 3;
    		self.fg_ctx.strokeStyle = "#ffa500";
    		var x = ignore[i][0][0] * self.sqSize + (self.sqSize / 2) + 1;
    		var y = ignore[i][0][1] * self.sqSize + (self.sqSize / 2) - 1
    		self.fg_ctx.moveTo(x, y);
    		self.fg_ctx.lineTo(x + (self.sqSize * ignore[i][1][0]), y + (self.sqSize * ignore[i][1][1]));
    	}
    	self.fg_ctx.stroke();*/
    ...
    setAlgorithm = function(algorithm) {
        ...
        if (algorithm == 'jps') { self.search = Search_Student_JPS(self.map, self.config); }
    ...
    self.setHTML = function() {
        ...
        self.controlDiv.innerHTML += "Search Algorithm: <select id='algorithm' onchange='setAlgorithm(value)';> \
                                        <option value='jps'>Student JPS (JPS)</option> \
